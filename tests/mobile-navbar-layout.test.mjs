...
test('More hub links to Bills, Categories and Stock', () => {
...
test('secondary mobile pages keep More selected', () => {
  assert.match(navigation, /const morePages = new Set\(\['limits', 'stock', 'subscriptions', 'bills'\]\)/);
...
test('light summary cards and navbar use symmetric mobile geometry', () => {
  assert.match(css, /\.m-inout-pills\s*\{[^}]*repeat\(2,minmax\(0,1fr\)\)/s);
  assert.match(css, /\.m-pill\s*\{[^}]*min-width:0[^}]*min-height:54px/s);
  assert.match(css, /#mn-home\s*\{\s*grid-column:1/);
  assert.match(css, /#mn-transactions\s*\{\s*grid-column:2/);
  assert.match(css, /#mn-analytics\s*\{\s*grid-column:4/);
